    ...
    if not service:
        ...
        def apply_events(snapshot, events):
            if 'sequence' in snapshot.columns and 'sequence' in events.columns:
                ...
                snap_ts_ns = int(snapshot["timestamp"].max().timestamp() * 1_000_000_000)
                events["event_ts_ns"] = (events["timestamp"] * 1_000_000 + events["timestampNanoseconds"]).astype('Int64')
        ...
        if 'order_book' in st.session_state:
            ...
            if st.button("Apply New Events"):
                ...
                try:
                    ...
                    elif order_book_df.index.name == 'timestamp':
                        last_ts = pd.to_datetime(order_book_df.index.max(), unit='ms', utc=True)
                    else:
                        raise ValueError("Cannot determine last timestamp from snapshot. No 'timestamp' column or index.")

                    if last_ts.year < 2000:
                        raise ValueError(f"Invalid timestamp extracted: {last_ts.isoformat()}")

                    st.info(f"Fetching events after snapshot timestamp: {last_ts.isoformat()}")
                    ...
                    st.error(f"Failed to apply new events: {e}")
